...
class ScrollyVideo {
  ...
  isSafari: boolean;
  currentTime = 0; // Saves the currentTime of the video, synced with this.video.currentTime
  targetTime = 0; // The target time before a transition happens
  canvas: HTMLCanvasElement | null = null; // The canvas for drawing the frames decoded by webCodecs
  context: CanvasRenderingContext2D | null = null; // The canvas context
  frames: ImageBitmap[] = []; // The frames decoded by webCodecs
  frameRate = 0;
  ...
  constructor({
    ...
    frameThreshold?: number | undefined;
    cover?: boolean | undefined;
    sticky?: boolean | undefined;
    full?: boolean | undefined;
    trackScroll?: boolean | undefined;
    lockScroll?: boolean | undefined;
    useWebCodecs?: boolean | undefined;
    ...
    if (typeof document !== "object") {
      throw new Error("ScrollyVideo must be initiated in a DOM context");
    }
    ...
    if (!src) {
      throw new Error("Must provide valid video src to ScrollyVideo");
    }
    ...
    this.updateScrollPercentage = (jump?: any) => {
      ...
      const containerBoundingClientRect = parentNode.getBoundingClientRect();
   ...
   */
  setVideoPercentage(percentage: number, options = {}) {
   ...
   * @param el
   */
  setCoverStyle(el: unknown) {
    if (this.cover) {
      if (!(el instanceof HTMLVideoElement)) {
        throw new Error(
          `setCoverStyle() called on with a wrong argument. Expected: HTMLVideoElement, received: ${el}`,
        );
      }
   ...
   * @param frameNum
   */
  paintCanvasFrame(frameNum: number) {
  ...
  transitionToTargetTime({
    ...
    const tick = ({
      ...
      //   throw new Error(`'easedProgress' is null`);
      // }
  ...
  setScrollPercent(percentage: number) {
    ...
    if (!parent || !(parent instanceof Element)) {
      throw new Error(`container's parentNode is not a valid Element`);
    }
